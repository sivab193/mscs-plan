...
function App() {
  ...
  const offerings = useMemo(() => sections.filter(s => s.term === term && `${s.courseCode} ${s.title} ${s.instructor}`.toLowerCase().includes(query.toLowerCase())).sort(compareCourseCode), [term, query])
